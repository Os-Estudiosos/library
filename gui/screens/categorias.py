...
class Categories(Screen):
    ...
    def build(self, **kwargs):
        ...
        if pagination:
            table = Table(self.app, "edit_categories", TablesManager.categoriaTable, "idcat", "see_category")
...
class EditCategory(Screen):
    ...
    def build(self, **kwargs):
        self.current_idcat = kwargs["entry"]["idcat"]
...
class SeeCategory(Screen):
    ...
    def build(self, **kwargs):
        ...
        description = ctk.CTkLabel(
            title_frame,
            text=f"Livros associados à categoria {category.nomecat}",
            font=("Arial", 15, "bold"),
            text_color=Colors.SLATE.c_900,
            justify="left"
        )
        ...
        pagination = TablesManager.lipertencecatTable.read(filter={
            "idcat": int(category.idcat)
        }, qtd=self.items_per_page, pagina=page)

        if pagination:
            table = Table(self.app, "edit_books", TablesManager.lipertencecatTable, ("isbnliv", "idcat"))
            table.build(pagination)
        else:
            empty_frame = EmptyFrame(self.app)
            empty_frame.build()
