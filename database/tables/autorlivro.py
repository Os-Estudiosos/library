...
class AutorLivroTable(Connection.initialize):
    ...
    def create(self, autor_liv, isbn_liv):
        try:
            ...
            ON CONFLICT (AutorLiv, ISBNLiv) DO UPDATE SET
            AutorLiv = EXCLUDED.AutorLiv;
            """
            self.conn.cursor().execute(sql, (autor_liv, isbn_liv))
            self.conn.commit()
    ...
    def update(self, autor_liv, isbn_liv):
        try:
            sql = f"UPDATE {self.name} SET AutorLiv = %s WHERE ISBNLiv = %s;"
            self.conn.cursor().execute(sql, (autor_liv, isbn_liv))
            self.conn.commit()
    ...
    def delete(self, autor_liv, isbn_liv):
        try:
            ...
            if self.conn.cursor().rowcount == 0:
                print(f"{self.name} com Autor {autor_liv} e ISBN {isbn_liv} não encontrada.")
                return
            self.conn.commit()
